...
/* --------------------------------------------------------------------
 * 점검대상표(chars)에 따라 containsChars와 containsCharsOnly를 
 * 구분해서 사용해야 한다.
 * containsCharsOnly를 사용하여 name필드를 검사하면 입력값이 chars표에
 * 맞는 것이 하나라도 없으면 return false를 발생시킨다.
 * 모든 문자가 chars에 해당되야지만 true를 발생시킨다.
 * --------------------------------------------------------------------
 */
//---------------------------------------------------------------------
...
function memberUpDelCheckForm(memUpDelForm, selectVal)
{
	...
	if(selectVal == "UP")
	{
		...
		if(isNameCheck(memUpDelForm.name.value) == true) {
			alert("AAA이름에는 숫자나 특수문자를 입력할 수 없습니다.");
